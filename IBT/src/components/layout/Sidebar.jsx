...
import {
  ...
  SearchCheck, Bell, ChevronDown, LogOut, Settings,
  FileText, Bus, Clock, ArrowLeftFromLine, Search,
  Calendar, Upload, Eye, Edit, Trash2, Plus
} from "lucide-react";
...
const Sidebar = ({ sidebarExpanded, setSidebarExpanded, onMobileClose }) => {
  ...
  return (
    <div className={`flex flex-col bg-white border-r border-gray-200 shadow-lg transition-all duration-300 ${sidebarExpanded ? "w-64" : "w-20"}`}>
        ...
        <div className="space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon size={20} className={isActive ? "text-emerald-600" : "text-gray-500 group-hover:text-gray-700"} />
                {sidebarExpanded && (
                  <>
                    <span className="font-medium text-sm whitespace-nowrap">{item.label}</span>
                    {isActive && <div className="ml-auto w-1.5 h-1.5 bg-emerald-600 rounded-full"></div>}
                  </>
                )}
              </NavLink>
            );
      ...
      <div className="border-t border-gray-200 p-3 space-y-1">
        ...
        {sidebarExpanded ? (
          <div className="mt-4 p-3 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border border-gray-200">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-cyan-600 rounded-xl flex items-center justify-center text-white font-bold shadow-md">A</div>
          ...
          <div className="mt-4 flex justify-center">
            <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-cyan-600 rounded-xl flex items-center justify-center text-white font-bold shadow-md cursor-pointer hover:shadow-lg transition-all hover:scale-105">A</div>
          </div>
        )}
      </div>
    </div>
